...
//6，根据左子树链表是否为空返回的整体双向链表的头节点
    ...
    public TreeNode5 Convert(TreeNode5 root){
        ...
        TreeNode5 left=Convert(root.left);

        //2、定位到左子树链表的最后一个节点（左子树最右边的节点）
        TreeNode5 p=left;//创建一个临时节点P,用来遍历找到左链表的最后一个节点(左子树最右边的节点)，p初始化指向做左子树的根节点，
        ...
        TreeNode5 right=Convert(root.right);
